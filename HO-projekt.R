...
p7 <- ggplot(Svar3, aes(x=Svar, y=Procent, fill=Svar)) + geom_bar(stat = "identity", position = "dodge") + scale_y_continuous(limits = c(0,38), breaks = seq(0, 40, by = 5)) +
...
p8 <- ggplot(Svar4, aes(x=Svar, y=Procent, fill=Svar)) + geom_bar(stat = "identity", position = "dodge") + scale_y_continuous(limits = c(0,38), breaks = seq(0, 40, by = 5)) +
...
p9 <- ggplot(Svar5, aes(x=factor(Svar), y=Antal)) + geom_bar(stat = "identity", position = "dodge", fill="#1F78B4") + coord_flip() +
  labs(title = "Motivationen i Mobiliseringsværnepligten", subtitle = "Status efter 4 ugers uddannelse", x=NULL, y="Antal", caption = "Kilde: Egen undersøgelse", fill=NULL) + scale_y_continuous(limits = c(0,43), breaks = seq(0, 43, by = 5)) +
  theme(panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank()) + 
...
p10 <- ggplot(Svar6, aes(x=factor(Svar), y=Antal)) + geom_bar(stat = "identity", position = "dodge", fill="#1F78B4") + coord_flip() +
  labs(title = "Sandsynligheden for at anbefale Mobiliseringsværnepligten", subtitle = "Status efter 4 ugers uddannelse" , x=NULL, y="Antal", caption = "Kilde: Egen undersøgelse", fill=NULL) + scale_y_continuous(limits = c(0,43), breaks = seq(0, 43, by = 5)) +
  theme(panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank()) + 
  theme(legend.position="none") + th 

#ggsave("Anbefaling.pdf", plot = p10 , width = 20, height = 15, units = "cm", path = "C:/Users/Andreas/Dropbox/Forsvaret/HO/Afgangsprojekt/Data")

ggsave("mot+sand, sammen.pdf", plot = grid.arrange(p9,p10, ncol=2, nrow=1), width = 40, height = 20, units = "cm", path = "C:/Users/Andreas/Dropbox/Forsvaret/HO/Afgangsprojekt/Data")

#Uddybning
Uddybning$Emne <- str_wrap(Uddybning$Emne, width = 20)
Uddybning$Emne <- with(Uddybning, reorder(Emne, desc(Antal)))
  
p11 <- ggplot(Uddybning,aes(x = Emne, y = Antal, fill = Prioritet)) + geom_bar(stat = "identity", position = "dodge") + 
...
p12 <- ggplot(Ønske, aes(fill=factor(Prioritet, levels = c("5. Prioritet", "4. Prioritet", "3. Prioritet", "2. Prioritet", "1. Prioritet")), y=Antal, x=Faktorer)) + geom_bar(position="stack", stat="identity") + 
  ...
  theme(panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank()) + 
  theme(legend.position="bottom", axis.text.x = element_text(angle = 15)) + scale_fill_brewer(palette = "Blues") + guides(fill=guide_legend(reverse = T)) + th 
...
p14 <- ggplot(Udsagn1, aes(fill=factor(Vigtighed, levels = c("Ikke vigtigt", "Lidt vigtigt", "Moderat vigtigt", "Meget vigtigt", "Ekstremt vigtigt")), y=Antal, x=Udsagn)) + geom_bar(position="stack", stat="identity") + 
  labs(title = "Betydningen af udsagn for individet", x=NULL, y="Antal", caption = "Kilde: Egen undersøgelse", fill=NULL) + scale_y_continuous(breaks = seq(0, 110, by = 10)) +
  theme(panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank()) + 
  theme(legend.position="bottom", axis.text.x = element_text(angle = 15)) + scale_fill_brewer(palette = "Blues") + guides(fill=guide_legend(reverse = F)) + th
